...
const prisma = require("../config/prisma"); // FIX: Import the singleton
// REMOVED: sendInfraRequestEmail import (no longer needed here)
...
------------------------------------------- */
function getVisionModel(messages) {
...
// REFACTORED: This function no longer saves to DB or sends email.
// It returns the AI's response or the tool call.
async function continueInfrastructureChat(messages, contactInfo) {
  ...
  try {
    ...
    // AI is just asking another question. Send it back to the user.
    return {
      type: "message",
...
/* -------------------------------------------
   VISION ARCHITECT CHAT HANDLER
------------------------------------------- */
async function continueVisionChat(messages, contactInfo) {
...
async function generateFrequencyProfile(profileData) {
  const {
    fullName,
    location,
    personalBeliefs,
...
async function continueHealingChat(messages, packages, contactInfo) {
  const { name } = contactInfo;
